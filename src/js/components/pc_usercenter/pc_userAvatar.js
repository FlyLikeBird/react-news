...
export default class PCUserAvatar extends React.Component {
  ...
  handleBlur(){
    fetch(`/usr/editSign?user=${this.props.user.username}&description=${this.state.description}`)
  ...
  render(){
    var { username, level, registertime, userFans, userFollow } = this.props.user;
    var { visible, previewVisible, imgUrl, description, isEdit, levelRemain, levelNum  } = this.state;
    registertime = formatDate(parseDate(registertime));
