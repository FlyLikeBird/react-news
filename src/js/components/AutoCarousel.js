...
export default class AutoCarousel extends React.Component {
    ...
    render() {
        ...
        return(
            ...
            <div ref={container=>this.container = container} className="auto-carousel">
                {
                    data.length
                    ?
                    <div className="bg" style={{backgroundImage:`url(${data[currentIndex]})`}}>
                        {
                            data.map((item,index)=>(
                                <div 
                                    className={index===currentIndex?'float selected':'float'}
