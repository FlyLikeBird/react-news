...
export default class PCTopNewsIndex extends React.Component{
    ...
    componentDidMount(){
        var { onLoadScrollFunc, cols } = this.props;
                ...
                <Col span={20} style={{height:'100%'}}>
                    {
                        ...
                        <div className={style["topNews-container"]}>
                            ...
                            <div ref={} className={style['content-container']}>
                            {
                                data.map((item,index)=>(
                                    <div style={{width:'25%',display:'inline-block',backgroundColor:`#${Math.floor(Math.random()*999)}`,padding:'10px'}}>
                                        <div>{index}</div>
                                    </div>
                                ))
                            }
